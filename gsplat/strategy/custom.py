...
from .ops import inject_noise_to_position, inject_noise_to_position_levy, relocate, relocate_l1, sample_add


@dataclass
class CustomMCMCStrategy(Strategy):
    ...
    - Periodically teleport GSs with low opacity to a place that has high opacity and high loss.
    ...
    Examples:
        ...
        >>> from gsplat import CustomStrategy, rasterization
        >>> params: Dict[str, torch.nn.Parameter] | torch.nn.ParameterDict = ...
        >>> optimizers: Dict[str, torch.optim.Optimizer] = ...
        >>> strategy = CustomMCMCStrategy()
    ...
    def initialize_from_config(self, config: Dict[str, Any]):
        """Initialize parameters (separate from state, certain sections of code assume state is Dict[str, Tensor])"""
        self.noiseDistribution = config.noise_distribution
        self.noiseParams = {
            'noiseLocation': config.noise_location,
            'noiseScale': config.noise_scale,
            'ppfMinRange': config.ppf_min_range,
            'ppfMaxRange': config.ppf_max_range,
        }

        self.relocationType = config.relocation_type
    ...
    def step_post_backward(
        ...
        state['pixelL1Loss'] = state['pixelL1Loss'].to(params['means'].device)

        binoms = state["binoms"]
        pixelL1Loss = state['pixelL1Loss']
        ...
        if (
            ...
            # teleport GSs
            n_relocated_gs = self._relocate_gs(params, optimizers, info, binoms, pixelL1Loss)
            if self.verbose:
        ...
        # add noise to GSs
        if self.noiseDistribution == 'levy':
            inject_noise_to_position_levy(
                params=params, optimizers=optimizers, state={}, scaler=lr * self.noise_lr, noiseParams = self.noiseParams
            )
        else:
